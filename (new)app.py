...
def display_real_time_view(refresh_interval, config):
    ...
    if not real_time_data.empty:
        ...
        with st.expander("\U0001f4cb View Raw Data"):
            ...
            df_raw = real_time_data.sort_values('timestamp', ascending=False).head(100).sort_values('timestamp').reset_index(drop=True)
...
def main():
    ...
    time.sleep(refresh_interval)  # Wait for the time interval
    st.experimental_rerun()  # Refresh the app after the interval
